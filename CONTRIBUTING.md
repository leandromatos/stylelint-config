...
This repository follows conventional commits, lints with the shared ESLint config, and formats with Prettier.

## Local setup

```bash
git clone git@github.com:leandromatos/stylelint-config.git
cd stylelint-config
pnpm install
```
...
| Script                | Purpose                                     |
| --------------------- | ------------------------------------------- |
| `pnpm run build`      | Emit `src/index.d.ts` from the JSDoc.       |
...
Publishing runs over OIDC (trusted publishing), with no npm token. It needs a trusted publisher registered once on npm, under package settings → Trusted Publisher → GitHub Actions, pointing at `leandromatos` / `stylelint-config` / `deploy.yaml`. Without it the publish fails with a 404.
